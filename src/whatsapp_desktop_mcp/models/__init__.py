"""Public Pydantic surface for whatsapp-desktop-mcp.
...
code (Plans 02-05) imports exclusively from this module:
    ...
    from whatsapp_desktop_mcp.models import Chat, Message, Coverage, encode_cursor
...
this module and from ``whatsapp_desktop_mcp.models.doctor``.

Phase 2 extension (Plan 02-02)
==============================
Adds four send-side public names:

- :class:`AuditEntry` — re-exported FROM ``whatsapp_desktop_mcp.sender.audit`` so
...
- :class:`OffendingSource` — PYDANTIC re-shape of the dataclass in
  ``whatsapp_desktop_mcp.sender.cross_chat_quote``. The bridge helper
...
from whatsapp_desktop_mcp.models.cursor import (
    AnchorKind,
    CursorError,
    decode_cursor,
    encode_cursor,
)
from whatsapp_desktop_mcp.models.doctor import (
    DoctorReport,
    PermissionBucket,
    PermissionState,
    PermissionStatus,
