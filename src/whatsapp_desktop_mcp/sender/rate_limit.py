...
Backing store: ``~/Library/Application Support/whatsapp-desktop-mcp/rate-limit.db``
...
================================================
``WHATSAPP_DESKTOP_MCP_RATE_PER_MIN`` and ``WHATSAPP_DESKTOP_MCP_RATE_PER_DAY`` shift
...
a future evolution of ``whatsapp_desktop_mcp.paths.resolve_chatstorage_path``
...
=====================
This module imports from ``whatsapp_desktop_mcp.exceptions`` (for
:class:`RateLimitExceeded` / :class:`InvalidChatId`) and
``whatsapp_desktop_mcp.paths`` (for ``resolve_chatstorage_path``, consumed by
...
_DB_PATH = (
    Path.home() / "Library" / "Application Support" / "whatsapp-desktop-mcp" / "rate-limit.db"
)
...
_ENV_PER_MIN = "WHATSAPP_DESKTOP_MCP_RATE_PER_MIN"
_ENV_PER_DAY = "WHATSAPP_DESKTOP_MCP_RATE_PER_DAY"
