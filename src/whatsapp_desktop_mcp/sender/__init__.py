...
re-exported from :mod:`whatsapp_desktop_mcp.models.send` for ergonomic
``from whatsapp_desktop_mcp.sender import send_text, SendResult`` calls at
the tool tier.

D-24 EVOLVED REL-05 invariant
=============================
The sender package as a whole may import the read-side DB-connection
helper module ONLY (used by :mod:`whatsapp_desktop_mcp.sender.verify` for the
...
The guardrail modules (:mod:`whatsapp_desktop_mcp.sender.rate_limit`,
:mod:`whatsapp_desktop_mcp.sender.audit`,
:mod:`whatsapp_desktop_mcp.sender.cross_chat_quote`), the sender primitives
(:mod:`whatsapp_desktop_mcp.sender.deeplink`,
:mod:`whatsapp_desktop_mcp.sender.osascript_send`,
:mod:`whatsapp_desktop_mcp.sender.ax_assert`), and the post-hoc verifier
(:mod:`whatsapp_desktop_mcp.sender.verify`) are NOT in :data:`__all__` —
they remain importable via their full dotted paths
(e.g. ``from whatsapp_desktop_mcp.sender.audit import append``) but the
