...
============================================================
  ...
  STEP 1 (D-19) — read_only_mode gate. The tool body reads the LIVE
    ...
    boolean — per W-4 the from-import form ``from whatsapp_mcp.server
...
in ``from whatsapp_mcp.server import ...`` — that form captures the
boolean value at module-import time (BEFORE CLI's late
``server.read_only_mode = False`` assignment in ``--no-read-only``
mode). Instead we add ``from whatsapp_mcp import server`` separately
...
# acceptance grep ``grep -cE '^from whatsapp_mcp import server'`` returns 1.
# ruff I001 (import-sort) would otherwise rewrite the two adjacent
# from-imports as ``from whatsapp_mcp import reader, server``, which
...
from whatsapp_mcp.models import ConfirmationSchema, SendResult
...
from whatsapp_mcp.sender.audit import AuditEntry, body_sha256
from whatsapp_mcp.sender.cross_chat_quote import OffendingSource
...
# constant in Plan 02-02's audit.py (~/Library/Logs/whatsapp-mcp/audit.log).
