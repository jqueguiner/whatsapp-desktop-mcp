...
``~/Library/Application Support/whatsapp-mcp/fts.sqlite`` (mode 0600)
...
:func:`whatsapp_mcp.reader.connection.open_ro` helper.
...
:func:`whatsapp_mcp.reader.messages._project_messages`. The joinback
...
:mod:`whatsapp_mcp.reader.connection`,
:mod:`whatsapp_mcp.reader.schema_v1`,
:mod:`whatsapp_mcp.reader.messages`,
:mod:`whatsapp_mcp.paths`,
:mod:`whatsapp_mcp.time`, and
:mod:`whatsapp_mcp.models`. **Zero** imports from
``whatsapp_mcp.sender.*`` — verified by the structural grep gate in
...
# whatsapp-mcp/`` parent directory by convention — see
# ``sender/rate_limit.py`` for the sibling-sidecar pattern.
_DB_PATH: Path = Path.home() / "Library" / "Application Support" / "whatsapp-mcp" / "fts.sqlite"
...
def open_rw_fts(db_path: Path | str | None = None) -> Iterator[sqlite3.Connection]:
    ...
    Separate from :func:`whatsapp_mcp.reader.connection.open_ro` because
    ...
    :func:`whatsapp_mcp.sender.rate_limit._ensure_db`).
    """
...
async def fts5_search(
    ...
    """Async FTS5 search — mirrors :func:`whatsapp_mcp.reader.search.like_search` signature.
    ...
    Returns:
        list of :class:`whatsapp_mcp.models.Message`, ordered by FTS5
