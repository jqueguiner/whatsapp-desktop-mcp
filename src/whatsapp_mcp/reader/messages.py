...
the locked :class:`whatsapp_mcp.models.Message` shape.
...
into :func:`whatsapp_mcp.models.encode_cursor` (anchor_kind="z_sort")
to build ``next_cursor``. ``ZSORT`` is NEVER a public attribute on
:class:`whatsapp_mcp.models.Message` — exposing it would invite callers
...
from whatsapp_mcp.models import Jid, Message, MessageKind
    ...
    *,
    ...
    Args:
        row: A :class:`sqlite3.Row` from one of the message SQL
            ...
            same columns).
        media_root: Result of :func:`whatsapp_mcp.paths.resolve_media_root`.
...
async def window(
    ...
    this value into :func:`whatsapp_mcp.models.encode_cursor` with
    ...
    Args:
        ...
        include_deleted: When ``False`` (default), the SQL template
            inlines :data:`~whatsapp_mcp.reader.tombstones.TOMBSTONE_SQL_WHERE`.
    """
...
async def since(
    ...
    :func:`whatsapp_mcp.time.unix_to_cocoa`.
    """
    db_path = resolve_chatstorage_path()
    media_root = resolve_media_root()
    return await asyncio.to_thread(
