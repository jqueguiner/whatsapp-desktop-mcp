...
Backing store: ``~/Library/Logs/whatsapp-mcp/audit.log`` (mode ``0o600``).
...
=====================
This module imports from ``whatsapp_mcp.*`` only via standard
Pydantic — no reader, no exceptions, no models. The Pydantic class
defined here (:class:`AuditEntry`) is re-exported through
``whatsapp_mcp.models.__init__`` so the public tool tier imports it
