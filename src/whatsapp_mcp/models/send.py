...
- :class:`OffendingSource` — the PYDANTIC re-shape of the dataclass in
  ``whatsapp_mcp.sender.cross_chat_quote``. This is the
...
if TYPE_CHECKING:
    from whatsapp_mcp.sender.cross_chat_quote import (
...
class SendResult(BaseModel):
    ...
    ``audit_log_path`` echoes ``~/Library/Logs/whatsapp-mcp/audit.log``
...
def offending_source_to_pydantic(
    ...
    on ``whatsapp_mcp.sender.cross_chat_quote`` — preserving the layered
