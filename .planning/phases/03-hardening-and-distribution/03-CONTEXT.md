...
- **D-06:** **Extend `.github/workflows/release.yml`** with a new `pkg-build` job (downstream of `publish` PyPI job, runs only if cert secrets exist):
  ...
  2. Run `scripts/build-pkg.sh` (new) which stages the Python venv per D-03 and produces unsigned `whatsapp-desktop-mcp-{version}.pkg`.
...
- **D-21:** **`tested_versions.md` is generated by hand initially**, with 1 row reflecting the maintainer's current setup (WA 26.16.74 / macOS 26.4 / Z_VERSION 1). Future versions added either manually or via a `whatsapp-desktop-mcp dev record-tested-version` CLI helper (out-of-scope decision; v1.1 candidate).
...
- **D-24:** **Smoke suite uses the SAME `_isolate_live_state` fixture as Phase 2** so the structural sandbox guarantee is single-source-of-truth. Extension: add `monkeypatch.setattr("whatsapp_desktop_mcp.reader.search_fts5._DB_PATH", tmp_path / "fts.sqlite")` to the fixture. Real WhatsApp send still fires; guardrail + FTS persistence is sandboxed.
...
- **D-27:** **`whatsapp-desktop-mcp dev` CLI subcommand surface added (Claude's discretion v1.1 polish):** suggested `whatsapp-desktop-mcp dev reset-rate-limit` (clears `~/Library/Application Support/whatsapp-desktop-mcp/rate-limit.db`) and `whatsapp-desktop-mcp dev rotate-audit-log` (forces immediate rotation). Phase 3 ships the `dev reset-rate-limit` subcommand (unblocks the Phase 2 verification carry-over for live-test budget recovery); other `dev` subcommands deferred.
...
- `src/whatsapp_desktop_mcp/permissions/automation.py` — D-09 patched probe still in use
- `src/whatsapp_desktop_mcp/sender/rate_limit.py` — D-11 separate path pattern (FTS sidecar follows same convention)
- `src/whatsapp_desktop_mcp/sender/audit.py` — Phase 2 append pattern; Phase 3 D-25..D-26 extends with rotation
- `src/whatsapp_desktop_mcp/reader/connection.py` — RO WAL pattern; FTS sidecar uses its own `open_ro_fts` per D-16
- `src/whatsapp_desktop_mcp/reader/schema_v1.py` — `Z_VERSION` probe; D-20 extends with `supported_version_range`
- `src/whatsapp_desktop_mcp/models/doctor.py` — `DoctorReport`; D-20 extends `SchemaFingerprint` with `supported_version_range` + `degraded_mode_warning`
- `src/whatsapp_desktop_mcp/cli.py` — D-28 adds `--fts5-mode` + `--audit-log-max-bytes` + `dev reset-rate-limit` subcommand
- `src/whatsapp_desktop_mcp/server.py` — D-29 adds `fts5_mode: str = "auto"` module attribute
...
### Reusable Assets
- `whatsapp_desktop_mcp.sender.rate_limit` (Phase 2) — separate-sidecar-DB pattern; FTS sidecar follows same convention (different file, same lifecycle shape).
- `whatsapp_desktop_mcp.sender.audit.append` (Phase 2) — append-only writer; D-25 extends with size-based rotation in the same module.
- `whatsapp_desktop_mcp.reader.connection.open_ro` (Phase 1) — RO WAL pattern; FTS sidecar opens its OWN connection (own DB) but mirrors the pattern.
- `whatsapp_desktop_mcp.reader.schema_v1.probe_schema_version` (Phase 1) — Z_VERSION probe; D-20 extends `SchemaFingerprint` with version-range fields.
- `whatsapp_desktop_mcp.tools.search_messages` (Phase 1) — LIKE-based fallback; D-29 dispatcher keeps it as the `disable` mode + the auto-mode fallback when sidecar absent.
...
- `whatsapp_desktop_mcp.cli` argparse setup (Phase 0/1) — pattern for adding `--fts5-mode` + `--audit-log-max-bytes` flags.
- `whatsapp_desktop_mcp.paths.system_settings_url` (Phase 0) — README D-32 TCC permission cards reuse this for the deep-link URLs.

### Established Patterns
- Stable-binary-path TCC grant insulation (D-03 P15 mitigation) — the `.pkg` lays down a launcher script at `/usr/local/bin/whatsapp-desktop-mcp` that invokes the bundled venv's interpreter, so TCC sees the SAME path across upgrades.
...
- Brew formula update (D-10) needs the PyPI `.tar.gz` SHA-256, computable via `brew install --build-from-source --HEAD` then `shasum -a 256` on the downloaded artifact, or via PyPI API: `curl -s https://pypi.org/pypi/whatsapp-desktop-mcp/0.1.0/json | jq -r '.urls[] | select(.packagetype=="sdist") | .digests.sha256'`.
- `whatsapp-desktop-mcp dev reset-rate-limit` (D-27) prompts for confirmation: "This will erase all rate-limit history at ~/Library/Application Support/whatsapp-desktop-mcp/rate-limit.db. Continue? [y/N]"; non-tty defaults to refuse.
...
- **`whatsapp-desktop-mcp dev rotate-audit-log` subcommand** — Claude's discretion; trivial after D-25 rotation lands
- **`whatsapp-desktop-mcp dev record-tested-version` subcommand** — v1.1 candidate; manual `tested_versions.md` edit suffices for v1.0
