...
dependency_graph:
  ...
  provides:
    - "FastMCP stdio server module (`whatsapp_mcp.server.mcp` + `run()`) with zero registered tools, stderr-first logging"
    - "argparse CLI dispatcher (`whatsapp_mcp.cli:main`) — `--version` / `--help` exit before FastMCP loads (lazy server import)"
    - "`python -m whatsapp_mcp` shim (`whatsapp_mcp.__main__`) — identical behavior to console script"
    ...
    - "Pure path resolver: `whatsapp_mcp.paths.resolve_chatstorage_path() -> str`"
    - "Single insertion-site comment in `server.py` line 44 marking the exact line Plan 03 will append (`from whatsapp_mcp.tools import doctor as _doctor`)"
  affects:
    ...
    - "Plan 04 (test suite) — `tests/unit/test_stdout_purity.py` will spawn `python -m whatsapp_mcp` (the shim shipped here); `tests/unit/test_exceptions.py` will introspect the frozen exception surface"
    - "Phase 1 readers/senders — will `from whatsapp_mcp.exceptions import FullDiskAccessRequired` etc. against an immutable surface; `paths.resolve_chatstorage_path` is the single source of truth for the DB path string"
tech_stack:
  added:
  ...
  patterns:
    ...
    - "Lazy server import inside `cli.main()`: `from whatsapp_mcp.server import run` is deferred until after argparse parses, so `--version` and `--help` exit before FastMCP loads"
...
key_files:
  ...
  modified: []
decisions:
  - "Plan 03 insertion-site comment is anchored at `src/whatsapp_mcp/server.py:44` — Plan 03's executor edits exactly that line, replacing the comment with `from whatsapp_mcp.tools import doctor as _doctor  # noqa: E402, F401`"
  - "Path resolver lives at `whatsapp_mcp/paths.py` (top-level, not inside `permissions/`) so Phase 1's read tools can import it without crossing into the `permissions/` namespace"
  - "Server module docstring rewritten to describe the `transport=` prohibition without using the literal `transport=` token, so the plan's `! grep -E '\\btransport\\s*=' src/whatsapp_mcp/server.py` acceptance criterion passes (was a near-miss the first verification surfaced)"
  - "`__init__.py` left at exactly `__version__ = \"0.1.0\"` (Plan 01 shipped this; Plan 02 explicitly re-uses, no `__all__` added — keeps the package root import-cheap so `import whatsapp_mcp` produces zero stdout bytes, which is the package-level P-PHASE0-01 invariant Phase 1 will rely on)"
metrics:
...
Landed the executable spine — FastMCP stdio server (zero tools, stderr-FIRST logging, no `transport=` argument), argparse CLI with lazy server import, `python -m whatsapp_mcp` shim, the frozen 5-class `WhatsAppMCPError`/`PermissionRequired`/`*Required` exception hierarchy, the Pydantic v2 `DoctorReport`/`PermissionStatus` contracts (Literal-typed enums, `all_granted` as a `@property` not a field), and the pure `resolve_chatstorage_path()` resolver — so `uv run whatsapp-mcp --version` works end-to-end and Plan 03 can register `doctor` against an immutable contract surface by appending a single line at `server.py:44`.
...
├── cli.py                     # NEW — argparse dispatcher; lazy `from whatsapp_mcp.server import run`
├── server.py                  # NEW — FastMCP("whatsapp-mcp"); logging.basicConfig stderr FIRST
...
`from whatsapp_mcp.server import run` is **lazy** (inside `main`) so `--version` / `--help` exit before FastMCP imports.

### Plan 03 insertion site (the single line Plan 03 appends)

`src/whatsapp_mcp/server.py:44` currently reads:

```python
# Phase 0 Plan 03 inserts: from whatsapp_mcp.tools import doctor as _doctor  # noqa: E402, F401
```

Plan 03's executor edits exactly this line, replacing the `# Phase 0 Plan 03 inserts: ` prefix with the actual import. The line is positioned AFTER `mcp = FastMCP(...)` (line 42) and BEFORE `def run()` (line 47), which is the P-PHASE0-06 ordering required to avoid a circular import (`tools/doctor.py` does `from whatsapp_mcp.server import mcp`).
...
| Step | Command                                                                                                                        | Result                                                                                                              |
| ---- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| 1    | `uv run whatsapp-mcp --version`                                                                                                | `whatsapp-mcp 0.1.0` (matches `^whatsapp-mcp 0\.1\.0$`); exit 0                                                     |
| 2    | `uv run python -m whatsapp_mcp --version`                                                                                      | `whatsapp-mcp 0.1.0`; exit 0 (identical to step 1 — confirms `__main__.py` shim)                                    |
...
| 5    | `python -c "from whatsapp_mcp.exceptions import FullDiskAccessRequired, ...; from whatsapp_mcp.models.doctor import ...; from whatsapp_mcp.paths import resolve_chatstorage_path; from whatsapp_mcp.server import mcp, run"` | Exits 0 — every interface published in `<context>` is importable                                                    |
| 6    | `python -c "from whatsapp_mcp.server import mcp" 2>/dev/null \| wc -c`                                                         | `0` — server import emits zero bytes on stdout (P-PHASE0-01 invariant evidence)                                     |

Additional task-level acceptance criteria (sampled — all passed):

- `! grep -E '^from fastmcp ' src/whatsapp_mcp/server.py` → no match (correct distribution import)
- `! grep -E '\btransport\s*=' src/whatsapp_mcp/server.py` → no match (no executable nor docstring `transport=` occurrence; the docstring was rewritten to avoid the literal token after the first verification surfaced the near-miss)
- `from whatsapp_mcp.cli import main; inspect.signature(main)` → `(argv: 'list[str] | None' = None) -> 'int'`
- `python -c "import whatsapp_mcp" 2>/dev/null | wc -c` → `0` (package root is import-cheap and stdout-clean)
...
| 3    | feat | `e446e3d` | feat(00-02): add CLI entry point and python -m whatsapp_mcp shim                   |
...
- **Files modified:** `src/whatsapp_mcp/server.py` (docstring lines only; no executable changes).
...
| --------- | ----------------- |
| **T-00-04** (stray stdout corrupts JSON-RPC) | Mitigated — `logging.basicConfig(stream=sys.stderr, ...)` is the first executable statement in `server.py`; `import whatsapp_mcp` and `from whatsapp_mcp.server import mcp` both produce zero stdout bytes (verified during plan-level verification step 6). The CI-side gate (`tests/unit/test_stdout_purity.py`) lands in Plan 04. |
...
| **T-00-07** (CLI traceback to stdout corrupts JSON-RPC) | Mitigated — argparse writes to stderr by default; `--version` exits via `argparse → SystemExit` BEFORE the lazy `from whatsapp_mcp.server import run` even fires; uncaught exceptions in `main()` propagate to Python's runtime which writes to stderr. |
...
- **Insertion-site comment in `server.py:44`** is intentional, not a stub. It documents (in code) the single-line edit Plan 03 will make. Plan 03's executor replaces the comment with the actual `from whatsapp_mcp.tools import doctor as _doctor  # noqa: E402, F401` import. The `tools/list` response of the Plan 02 server is `[]` (empty array) — also intentional; Plan 04's stdout-purity test must accommodate this, and its plan already does.
...
```
git log --oneline -3
e446e3d feat(00-02): add CLI entry point and python -m whatsapp_mcp shim   ✓ FOUND
...
```
src/whatsapp_mcp/exceptions.py     ✓ FOUND
src/whatsapp_mcp/models/doctor.py  ✓ FOUND
src/whatsapp_mcp/paths.py          ✓ FOUND
src/whatsapp_mcp/server.py         ✓ FOUND
src/whatsapp_mcp/cli.py            ✓ FOUND
src/whatsapp_mcp/__main__.py       ✓ FOUND
```

Behavioral spot-checks:

- `uv run whatsapp-mcp --version` → `whatsapp-mcp 0.1.0` ✓
- `uv run python -m whatsapp_mcp --version` → `whatsapp-mcp 0.1.0` ✓
- `uv run ruff check src tests` → All checks passed ✓
- `uv run mypy` → 16 source files clean ✓
- `python -c "import whatsapp_mcp" 2>/dev/null | wc -c` → 0 ✓
- `python -c "from whatsapp_mcp.server import mcp" 2>/dev/null | wc -c` → 0 ✓
