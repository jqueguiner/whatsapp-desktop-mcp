...
provides:
  - whatsapp_desktop_mcp.reader.open_ro
  - whatsapp_desktop_mcp.reader.SUPPORTED_VERSIONS
  - whatsapp_desktop_mcp.reader.is_supported
  - whatsapp_desktop_mcp.reader.probe_z_version
  - whatsapp_desktop_mcp.reader.is_tombstone
  - whatsapp_desktop_mcp.reader.resolve_media_ref
  - whatsapp_desktop_mcp.reader.list_chats
  - whatsapp_desktop_mcp.reader.find_chat_by_id
  - whatsapp_desktop_mcp.reader.find_chat_by_jid
  - whatsapp_desktop_mcp.reader.window
  - whatsapp_desktop_mcp.reader.since
  - whatsapp_desktop_mcp.reader.context_around_stanza
  - whatsapp_desktop_mcp.reader.parent_of_stanza
  - whatsapp_desktop_mcp.reader.latest_timestamp
  - whatsapp_desktop_mcp.reader.get_group_info
  - whatsapp_desktop_mcp.reader.get_members
  - whatsapp_desktop_mcp.reader.search_contacts
  - whatsapp_desktop_mcp.reader.resolve_lid_to_phone
  - whatsapp_desktop_mcp.reader.resolve_phone_to_lid
  - whatsapp_desktop_mcp.reader.like_search
affects:
...
key-files:
  created:
    ...
    - src/whatsapp_desktop_mcp/reader/media.py
    - src/whatsapp_desktop_mcp/reader/messages.py
    - src/whatsapp_desktop_mcp/reader/chats.py
    - src/whatsapp_desktop_mcp/reader/groups.py
    - src/whatsapp_desktop_mcp/reader/contacts.py
...
The 10-file `whatsapp_desktop_mcp.reader` package that owns ALL SQL knowledge in
...
the entire `reader/` tree imports zero symbols from `whatsapp_desktop_mcp.sender`.

## What Shipped

### Foundations (Task 1)

**`src/whatsapp_desktop_mcp/reader/connection.py`** — `open_ro(db_path) -> Iterator[sqlite3.Connection]`
...
**`src/whatsapp_desktop_mcp/reader/schema_v1.py`** — `SUPPORTED_VERSIONS = frozenset({1})`,
...
**`src/whatsapp_desktop_mcp/reader/tombstones.py`** — `is_tombstone(message_type, flags, text) -> bool`
...
**`src/whatsapp_desktop_mcp/reader/media.py`** — `resolve_media_ref(row, media_root) -> MediaRef | None`
...
file-wide grep gate `grep -rcE 'ZMEDIAKEY|ZMETADATA|ZRECEIPTINFO' src/whatsapp_desktop_mcp/reader/`
returns 0 across every file in the package.

### Data Accessors (Task 2)

**`src/whatsapp_desktop_mcp/reader/messages.py`** — owns `_parse_jid`,
...
**`src/whatsapp_desktop_mcp/reader/chats.py`** — `async def list_chats(limit=200) -> list[Chat]`,
...
**`src/whatsapp_desktop_mcp/reader/groups.py`** — `async def get_group_info(chat_id) -> GroupInfo | None`,
...
**`src/whatsapp_desktop_mcp/reader/contacts.py`** — `async def search_contacts(query, limit=20) -> list[Contact]`
...
**`src/whatsapp_desktop_mcp/reader/search.py`** — `async def like_search(query, chat_id=None, sender_jid=None, before=None, after=None, limit=50, include_deleted=False) -> list[Message]`.
...
**`src/whatsapp_desktop_mcp/reader/__init__.py`** — replaces the empty Phase 0
...
| **W5** | `GroupInfo.description = None` and `is_muted = False` hard-coded for v0.1; no execute-time PRAGMA probe; no TODO comment | `groups._get_group_info_blocking` builds the model with the literals. `grep -rE 'PRAGMA table_info' src/whatsapp_desktop_mcp/reader/` returns 0. `grep -rE '# TODO' src/whatsapp_desktop_mcp/reader/groups.py` returns 0. |
...
| `ZMEDIAKEY|ZMETADATA|ZRECEIPTINFO` | `src/whatsapp_desktop_mcp/reader/` (recursive) | 0 | =0 (T-02-05) |
| `whatsapp_desktop_mcp\.sender` (any from/import) | `src/whatsapp_desktop_mcp/reader/` (recursive) | 0 | =0 (REL-05) |
| `f"SELECT|f".*FROM ZWA` (f-string SQL gate) | `src/whatsapp_desktop_mcp/reader/` (recursive) | 0 | =0 (T-02-01) |
| `asyncio\.to_thread` | `src/whatsapp_desktop_mcp/reader/` (recursive) | 22 | ≥8 |
...
- `from whatsapp_desktop_mcp.reader import list_chats, window, since, context_around_stanza, get_group_info, get_members, search_contacts, like_search, latest_timestamp, resolve_lid_to_phone, resolve_phone_to_lid, parent_of_stanza, find_chat_by_id, find_chat_by_jid` succeeds; all 14 are `iscoroutinefunction` (verified via inline introspection).
- `from whatsapp_desktop_mcp.reader import open_ro, probe_z_version, SUPPORTED_VERSIONS, is_supported, is_tombstone, resolve_media_ref` succeeds (the 5 plumbing helpers + `SUPPORTED_VERSIONS` data attribute).
...
- [x] All 10 reader files exist; `from whatsapp_desktop_mcp.reader import ...` for the 20 names in `__all__` succeeds.
...
- [x] `! grep -rE "(?:from|import)\s+whatsapp_desktop_mcp\.sender" src/whatsapp_desktop_mcp/reader/` succeeds (REL-05 — no sender imports anywhere in reader/).
- [x] `! grep -nE 'f"SELECT|f".*FROM ZWA' src/whatsapp_desktop_mcp/reader/` succeeds (no f-string SQL — parameterized queries only).
- [x] `grep -rE "asyncio\.to_thread" src/whatsapp_desktop_mcp/reader/` returns 22 (well over the ≥8 threshold).
...
- [x] No imports from `whatsapp_desktop_mcp.sender` anywhere in `src/whatsapp_desktop_mcp/reader/`.
- [x] No `executemany|INSERT|UPDATE|DELETE` against the WhatsApp DB in `src/whatsapp_desktop_mcp/reader/`. Verified: `grep -rE 'executemany|INSERT|UPDATE|DELETE' src/whatsapp_desktop_mcp/reader/` returns 0 matches.
- [x] `ruff check src/whatsapp_desktop_mcp/reader/` clean; `ruff format --check` clean; `mypy src/whatsapp_desktop_mcp/reader/` (strict) clean.
...
- **Issues:**
  - (a) Initial `connection.py` docstring used the literal token `immutable=1` twice (in the "why not" rationale comment + the `def open_ro` docstring), which triggered the `grep -cE 'immutable' src/whatsapp_desktop_mcp/reader/connection.py` "must return 0" gate (CLAUDE.md hard rule #3).
  - (b) Initial `media.py` docstring + `resolve_media_ref` docstring named the encrypted/protobuf BLOB columns directly (`ZMEDIAKEY` / `ZMETADATA`), which triggered the `grep -cE 'ZMEDIAKEY|ZMETADATA' src/whatsapp_desktop_mcp/reader/media.py` "must return 0" gate (DATA-04).
  - (c) Initial `schema_v1.py` had two comments that named the same forbidden BLOB columns directly, which broke the threat-model file-wide `grep -rcE 'ZMEDIAKEY|ZMETADATA|ZRECEIPTINFO' src/whatsapp_desktop_mcp/reader/` "must return 0 across every file" gate (T-02-05).
...
- **Files modified:** `src/whatsapp_desktop_mcp/reader/connection.py`, `src/whatsapp_desktop_mcp/reader/media.py`, `src/whatsapp_desktop_mcp/reader/schema_v1.py`.
...
- **Files modified:** `src/whatsapp_desktop_mcp/reader/messages.py`.
...
- **Files modified:** `src/whatsapp_desktop_mcp/reader/media.py`.
...
- **T-02-04** (writes to WhatsApp's DB): mitigated structurally by `?mode=ro` (SQLite refuses writes). No `INSERT` / `UPDATE` / `DELETE` / `VACUUM` / `executemany` anywhere in `src/whatsapp_desktop_mcp/reader/`.
...
- **T-02-07** (sender cross-imports): mitigated by static gate. `grep -rE 'whatsapp_desktop_mcp\.sender' src/whatsapp_desktop_mcp/reader/` returns 0.
