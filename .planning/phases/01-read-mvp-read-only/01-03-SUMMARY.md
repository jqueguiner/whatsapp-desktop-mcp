...
requires: [phase-0]
provides:
  - whatsapp_mcp.exceptions.ReadOnlyMode
  - whatsapp_mcp.server.read_only_mode
  - cli.--read-only-flag
...
### `src/whatsapp_mcp/exceptions.py` (modified)
...
### `src/whatsapp_mcp/server.py` (modified)
...
### `src/whatsapp_mcp/cli.py` (modified)
...
`whatsapp-mcp --help` now renders the flag in argparse's mutually-exclusive
shorthand: `[--read-only | --no-read-only]`. `whatsapp-mcp --version` still
...
| `^from whatsapp_mcp\.tools import doctor as _doctor` | `server.py` | 1 (Phase 0 preserved) |
...
| `from whatsapp_mcp\.server import run` | `cli.py` | 1 (lazy import preserved exactly once) |
| `^\s*run\(\)\s*$` | `cli.py` | 1 (Phase 0 entry-point preserved) |

## Behavior Verification — all pass

- `from whatsapp_mcp.exceptions import ReadOnlyMode` succeeds.
...
- `from whatsapp_mcp.server import read_only_mode` returns `True` at import time (default).
- `mcp.list_tools()` returns exactly `['doctor']` (Plan 01-04 will add 7 more).
- `uv run whatsapp-mcp --version` prints `whatsapp-mcp 0.1.0` and exits 0 (before FastMCP loads).
- `uv run whatsapp-mcp --help` shows `[--read-only | --no-read-only]` in usage and exits 0.
...
- REL-05: `grep -r 'whatsapp_mcp.sender' src/whatsapp_mcp/{cli,server,exceptions}.py` returns 0 matches (no sender/ leak in the protocol-side surface).

## Acceptance Criteria — all met

- [x] `--read-only` and `--no-read-only` both parse without error; default is `--read-only` (= True) per the v0.1 carry-over decision.
- [x] `whatsapp_mcp.server.read_only_mode` is a module-level `bool` set by `cli.main()` before the FastMCP tool-registration imports trigger.
...
- **T-03-05** (`mitigate`): `ReadOnlyMode` is a 2-line subclass with no `__init__` override; zero I/O at import time. Verified: `grep -cE 'open\(|subprocess|requests' src/whatsapp_mcp/exceptions.py` returns 0.
...
>
> Insertion point: `src/whatsapp_mcp/server.py` — append the gated import block AFTER the Plan 01-04 read-tool import block (a marker comment is already in place). The flag will read `True` by default; users must explicitly pass `--no-read-only` to enable sends.
>
> The `ReadOnlyMode` exception class is already minted in `whatsapp_mcp.exceptions` (sibling of `PermissionRequired`); Phase 2's `send_message` can `from whatsapp_mcp.exceptions import ReadOnlyMode` without forward-declaration concerns and raise it as `raise ReadOnlyMode("send_message is disabled because server is in --read-only mode")` from within the tool body if the flag flips post-startup (e.g. by test injection).
